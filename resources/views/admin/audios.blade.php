        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="open" class="h-screen fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center">
                ...
                <div class="flex justify-between pb-10">
                    <h2 class="text-5xl font-bold mb-4 UNDERTOWNPERSONALUSE">Add new audio</h2>
                    <!-- Close button -->
                    <button @click="open = false" class="mt-4 px-4 py-2 bg-black text-white rounded">Close</button>
                </div>
                <hr>
                    <!-- Form -->
                <form action="{{ route('audio.store') }}" class="pt-10" method="POST"  enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mt-6">
                        ...
                        <input type="file" name="image_path" id="image_path" class="w-full px-4 py-2 mb-4 border rounded-md focus:outline-none focus:border-blue-500" required/>
                ...
                <div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 p-6 lg:p-8">
                    ...
                    <div>
                        <div>
                            <img src="{{ asset('storage/images/' . $audio->image_path) }}" style="width:200px;" alt="">
                        </div>
                        ...
                        <p class="mt-4 text-sm flex justify-between">
                            <a class="py-2" href="{{ route('audio.edit', $audio->id) }}" class="inline-flex items-center font-semibold text-black">
                              Edit
                            </a>
                            <hr >
                            <form class="py-2" method="POST" action="{{ route('audio.destroy', $audio->id) }}" onsubmit="return confirm('Are you sure you want to delete this item?');">
                            @csrf
                            @method('delete')
